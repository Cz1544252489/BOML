...
def build(
    ...
    hyper_repr_model = boml_ho.meta_learner(
        _input=exs[0].x,
        dataset=metasets,
        meta_model="V1",
        name=method,
    ...
    for k, ex in enumerate(exs):
        ...
        ex.optimizers["apply_updates"], _ = boml.BOMLOptSGD(learning_rate=lr0).minimize(
            ex.errors["training"], var_list=ex.model.var_list
        )
    ...
    saver = tf.train.Saver(
        tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES), max_to_keep=10
    )
...
def train_and_test(
    ...
    copyfile(
        executing_file_path, os.path.join(exp_dir, executing_file_path.split("/")[-1])
    )

    exs, boml_ho, saver = build(
        metasets,
        learn_lr,
        learn_alpha,
        learn_alpha_itr,
...
def test_meta_feat():
    ...
    weights_initializer = (
        tf.contrib.layers.xavier_initializer() if args.xavier else tf.zeros_initializer
    )
    ...
    if args.mode == "train":
        train_and_test(
            ...
            learn_alpha_itr=args.learn_alpha_itr,
            learn_st=args.learn_st,
            mlr0=args.meta_lr,
            mlr_decay=args.meta_lr_decay_rate,
            T=args.T,
            resume=args.resume,
